...
test_that("k_means",
          {
            expect_equal(k_means(testdata1, 2, means = matrix(c(1,1,7,6), 2, byrow = TRUE))[["cluster"]], cluster1)
            expect_equal(k_means(testdata2, 2, means = matrix(c(1,1,0,7,6,0), 2, byrow = TRUE))[["cluster"]], cluster2)
            expect_equal(k_means(testdata3, 3, means = matrix(c(1,1,7,6,1,7), 3, byrow = TRUE))[["cluster"]], cluster3)
            expect_equal(k_means(testdata4, 3, means = matrix(c(1,1,7,6,1,7), 3, byrow = TRUE))[["cluster"]], cluster4)
            expect_equal(k_means(testdata5, 3, means = matrix(c(1,1,7,6,1,7), 3, byrow = TRUE))[["cluster"]], cluster5)
            expect_error(k_means(testdata6, 3, means = matrix(c(1,1,7,6,1,7), 3, byrow = TRUE))[["cluster"]], "Strings cannot be clustered.")
            expect_error(k_means(testdata7, 3, means = matrix(c(1,1,7,6,1,7), 3, byrow = TRUE))[["cluster"]], "The given datatype is not supported.")
            expect_error(k_means(testdata8, 8, means = matrix(c(1,1,7,6,1,7), 3, byrow = TRUE))[["cluster"]], "More clusters required than given different datapoints. Every datapoint is his own cluster.")
            expect_error(k_means(testdata9, 3, means = matrix(c(1,1,7,6,1,7), 2, byrow = TRUE))[["cluster"]], "Wrong dimension or quantity of assigned means. Points are read by column here.")
          }
)
